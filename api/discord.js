// api/discord.js — Stable + Admin (kick/move) + Nicknames + Invisible STATE
import {
  InteractionType,
...
// ---- Invisible STATE by masked link ----
// 內容尾端放一個隱形超連結，裡面是 #STATE:<base64>，畫面完全不會顯示
const ZERO_WIDTH = "\u200B";
function encodeState(state) {
  const b64 = Buffer.from(JSON.stringify(state)).toString("base64");
  // 使用 masked link，不會產生預覽，也不會有文字
  return `\n[${ZERO_WIDTH}](https://s.local/#STATE:${b64})`;
}
...
async function fetchMemberDisplayName(guildId, userId) {
  ...
  try {
    ...
    const name = j.nick || j.user?.global_name || j.user?.username || userId;
    return name;
  } catch {
    return `<@${userId}>`;
  }
...
async function renderVisibleContent(state, guildId) {
  ...
  for (let i = 0; i < caps.length; i++) {
    ...
    if (arr.length === 0) {
      ...
      const names = await Promise.all(arr.map(uid => fetchMemberDisplayName(guildId, uid)));
      lines.push(`第${numToHan(i + 1)}團： ${names.join("、")}`);
    }
  }
...
export default async function handler(req, res) {
  ...
  if (i.type === InteractionType.APPLICATION_COMMAND) {
    if (i.data?.name === "cteam") {
      ...
      const content = await renderVisibleContent(state, i.guild_id);
      return res.status(200).json({
        type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
        data: {
          content,
    ...
    (async () => {
      try {
        ...
        if (action === "join") {
          ...
          const has = state.members[key].includes(userId);
          if (has) {
            await fetch(`https://discord.com/api/v10/webhooks/${i.application_id}/${i.token}`, {
        ...
        const newContent = await renderVisibleContent(state, i.guild_id);
  ...
  if (i.type === InteractionType.MODAL_SUBMIT) {
    ...
    let msg;
    try {
      msg = await getMessage(i.channel_id, targetMsgId);
    } catch (e) {
      return res.status(200).json({
        type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
    ...
    const allKeys = Object.keys(state.members);
    const inGroups = allKeys.filter(k => state.members[k].includes(targetUserId)).map(Number);
    ...
    if (action === "kick") {
      ...
      for (const k of inGroups) {
        const arr = state.members[String(k)];
        const pos = arr.indexOf(targetUserId);
